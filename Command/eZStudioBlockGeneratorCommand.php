...
/*
 * This file is base on the original symfony bundle generator command by Fabien Potencier
 *
...
use DKeZStudio\BlockGeneratorBundle\Command\GeneratorCommand;
use DKeZStudio\BlockGeneratorBundle\Command\eZStudioBlockBundleGenerator as BundleGenerator;
...
use DKeZStudio\BlockGeneratorBundle\Command\Validators as eZValidators;
    ...
    protected function configure()
    {
        $this
            ->setDefinition(array(
                ...
                new InputOption('blockname', '', InputOption::VALUE_REQUIRED, 'Name of the eZStudio Block'),
                new InputOption('fieldsnumber', '', InputOption::VALUE_REQUIRED, 'Number of fields for the eZStudio Block'),
                new InputOption('type', '', InputOption::VALUE_REQUIRED, 'Type of each fields of the eZStudio Block'),
            ))
            ->setDescription('Generates a eZ Studio Block in a Symfony Bundle')
            ->setHelp(<<<EOT
The <info>generate:ezstudio:block</info> command helps you generates new eZ Studio Block in bundles.
...
<info>php app/console generate:ezstudio:block --namespace=Acme/BlogBundle</info>
...
<info>php app/console generate:ezstudio:block --namespace=Acme/BlogBundle --dir=src [--bundle-name=...] --no-interaction</info>

Note that the bundle namespace must end with "Bundle".
EOT
            )
            ->setName('generate:ezstudio:block')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $blockname = eZValidators::validateBlockName($input->getOption('blockname'));
        $fieldsnumber = eZValidators::validateFieldsNumber($input->getOption('fieldsnumber'));
        ...
        $generator->generate($namespace, $bundle, $dir, $format, $structure, $blockname, $fieldsnumber, $type);
    ...
    {
        $questionHelper = $this->getQuestionHelper();
        $questionHelper->writeSection($output, 'Welcome to the eZ Studio block generator');
        ...
        $blockname = null;
        try {
            $blockname = $input->getOption('blockname') ? eZValidators::validateBlockName($input->getOption('blockname')) : null;
        } catch (\Exception $error) {
            $output->writeln($questionHelper->getHelperSet()->get('formatter')->formatBlock($error->getMessage(), 'error'));
        }
        ...
        if (null === $blockname) {
            ...
            $question->setValidator(
                array('DKeZStudio\BlockGeneratorBundle\Command\Validators', 'validateBlockName')
            );
            $blockname = $questionHelper->ask($input, $output, $question);
            $input->setOption('blockname', $blockname);
        }

        //eZStudio Block fields number
        $fieldsnumber = null;
        try {
            $fieldsnumber = intval($input->getOption('fieldsnumber')) ? eZValidators::validateFieldsNumber($input->getOption('fieldsnumber')) : null;
        } catch (\Exception $error) {
            $output->writeln($questionHelper->getHelperSet()->get('formatter')->formatBlock($error->getMessage(), 'error'));
        }

        if (null === $fieldsnumber) {
            $output->writeln(array(
                '',
            ...
            $question->setValidator(
                array('DKeZStudio\BlockGeneratorBundle\Command\Validators', 'validateFieldsNumber')
            );
            $fieldsnumber = $questionHelper->ask($input, $output, $question);
            $input->setOption('fieldsnumber', $fieldsnumber);
        }

        //get the type for each eZ Studio block field
        $type = array();
        for($i = 1; $i <= intval($fieldsnumber); $i++) {
            $output->writeln(array(
                '',
                'Determine the format to use for the eZ Studio block field '.$i,
                '',
            ));
            $question = new Question($questionHelper->getQuestion('Field type for field '.$i.' (integer, string, text, multiple or embed)', null));
            $question->setValidator(
                array('DKeZStudio\BlockGeneratorBundle\Command\Validators', 'validateFieldsType')
            );
